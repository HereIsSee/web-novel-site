...
const NovelFollowCard = ({id, title, allRead=false})=>{
    ...
    return(
        <NovelCardLayout
            ...
            fitMode="hide"
        >
            <Link className={styles['novel-card-title']} to={`/novels/${id}/${novelSlog}`}>The Legend Of William Oh</Link>
            ...
            {allRead ? (
                <> 
                    ...
                    <Link className={`${styles["last-update"]} ${styles["last-read"]}}`} to={`/novels/${2}/chapters/${2}`}>
                ...
                <>
                    ...
                    <Link className={styles["last-update"]} to={`/novels/${2}/chapters/${2}`}>
                        <div>Chapter Name</div>
                        <div>19 hours ago</div>
                    </Link>

                    <div className={styles['novel-status']}>Last Read Chapter</div>
                    <Link className={styles["last-read"]} to={`/novels/${2}/chapters/${2}`}>
                        <div>Chapter Name</div>
                        <div>19 hours ago</div>
                    </Link>
                </>
